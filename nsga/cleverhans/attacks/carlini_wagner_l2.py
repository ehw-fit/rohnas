...
class CWL2(object):
  ...
  def attack(self, imgs, targets):
    ...
    r = []
    f  = open("RESULTS_CW.txt", "w")
    for i in range(0, len(imgs), self.batch_size):
      print(
          ("Running CWL2 attack on instance {} of {}".format(i, len(imgs))))
      r.extend(
          self.attack_batch(imgs[i:i + self.batch_size],
                            targets[i:i + self.batch_size],f))
    f.close()
    return np.array(r)

  def attack_batch(self, imgs, labs,f):
    """
    ...
    for outer_step in range(self.BINARY_SEARCH_STEPS):
      ...
      if outer_step == (self.BINARY_SEARCH_STEPS) -1:
                f.write("Successfully generated adversarial examples " +
                    "on {} of {} instances.  ".format(
                        sum(upper_bound < 1e9), batch_size))
                f.write("   Mean successful distortion: {:.4g}\n".format(mean))
