...
class KerasModelWrapper(Model):
  ...
  def _get_softmax_name(self):
    ...
    for layer in self.model.layers:
      ...
      if 'name' in cfg and cfg['name'] == 'capsnet':
        print(layer.name)
        return layer.name
  ...
  def _get_logits_name(self):
    ...
    print((softmax_layer._inbound_nodes[0].outbound_layer.name))
    ...
    if LooseVersion(tf.__version__) < LooseVersion('1.14.0'):
      logits_name = node.outbound_layer.name
    else:
      logits_name = node.outbound_layer.name
  ...
  def get_logits(self, x):
    ...
    if logits_name == self._get_softmax_name():
      ...
      logits_layer = softmax_logit_layer._op.outputs[0]
    ...
    if self.keras_model is None:
      ...
      if abstract_layers:
        warnings.warn(
            ...
            "This could happen due to using of stacked model." + str(abstract_layers))
